...
const addToCart = (func, selectedIDs = []) => {
	if(!cars.length){
		...
		if(!inStockCars.length){
			...
			const notInCartIDs = inStockCarsIDs.filter(id => !cartCarIDs.includes(id));
			if(!notInCartIDs.length){
				...
				inquirer.prompt([
					...
					.then(answers => {
						selectedIDs.push(answers.id);
						if(answers.continue && selectedIDs.length !== notInCartIDs.length){
							addToCart(func, selectedIDs);
						}
						else {
							let spinner = undefined;
							if(answers.continue){
								spinner = ora('All available cars were chosen, processing...').start();
							}
							else{
								spinner = ora('Processing...').start();
							}
							...
							setTimeout(() => {
								process.stdout.write('\r');
								spinner.succeed('Car/s successfully selected, and ready to be added to cart.');
								inquirer.prompt([
									{
										type: 'list',
										name: 'decision',
	...
	inquirer.prompt([
		...
		.then(answers => {
			...
			if(answers.continue && selectedIDs.length !== selectedCarsInfo.length){
				...
				let spinner = undefined;
				if(answers.continue){
					spinner = ora('All selections are set to be remove already...').start();
				}
				else{
						...
						inquirer.prompt([
							{
								...
								choices: ['Add selections to cart', 'Remove another car', 'Do not add anything to cart']
							}
						])
							.then(answer => {
								const spinner = ora('Processing...').start();
